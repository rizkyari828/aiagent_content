...
- Build Local AI Content Studio. Immediate request: prepare agent documentation and reusable repo context first.
...
.NET 10 modular monolith + React/TypeScript/Vite + PostgreSQL metadata + filesystem media. Ollama/Qwen3-8B quantized candidate; recorded human narration; faster-whisper multilingual small candidate; validated manifests rendered with FFmpeg. Versions, actual model digests, hardware suitability, and licenses still need implementation-time verification.

One persistent PostgreSQL job worker (may be BackgroundService); GPU-heavy concurrency one. Three version-bound approvals; critical QA failures block publication readiness. Export/import and restore are P1 requirements. Available local inputs must support offline production after setup.
