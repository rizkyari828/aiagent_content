...
- PASS - 275/275 tests (0 failed, 0 skipped) with `ConnectionStrings__DefaultConnection` set; 83 focused `AIStudio.Tests.Rendering` tests cover motion mapping/safety default, transition selection/duration, subtitle styling/burn-in, music/ducking/SFX argument generation, the SVG composer (all layouts + escaping), the librsvg rasterizer boundary, and backward compatibility when new options are absent.
