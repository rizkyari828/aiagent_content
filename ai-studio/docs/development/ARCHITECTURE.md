...
Cross-module rules are now frozen in **ADR-018 Extractable Module Boundaries** and **ADR-019 Domain-Specific Workflows and Proven Capability Reuse** (PRD addendum `docs/product/parts/13_EXTRACTABLE_MODULE_BOUNDARIES.md`, mirrored in `AGENTS.md`): modules are co-located by default but remotely extractable by contract; no cross-module persistence or Infrastructure access; cross-module interaction uses application-owned, transport-neutral, serializable contracts; extraction requires operational justification and replaces the transport adapter, not the business logic; and shared capabilities are extracted only after a second real workflow proves duplication. Assembly-level enforcement lives in `tests/AIStudio.Tests/Architecture/ArchitectureBoundaryTests.cs`.
