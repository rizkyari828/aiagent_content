...
- `tests/AIStudio.Tests/Rendering/SceneVisualPlannerTests.cs`, `SceneTimingTests.cs`, `FfmpegVideoRendererTests.cs`, `RenderVideoJobHandlerTests.cs`, `GenerateSceneVisualsJobHandlerTests.cs` — planner display-text/layout mapping, timing allocation + `SubtitleTimeline`, durable Manim duration from narration, and durable subtitle re-timing.
