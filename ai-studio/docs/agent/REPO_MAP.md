...
- `src/AIStudio.Application/Rendering/Visuals/IImageGenerationProvider.cs`, `ImageGenerationRequest.cs`, `src/AIStudio.Application/IdentityAssets/IdentityAssetResolution.cs`, `src/AIStudio.Infrastructure/Rendering/ComfyUiImageGenerationProvider.cs`, `ComfyUiOptions.cs`, `Rendering/ComfyUI/flux2_klein_4b_distilled.json`, `flux2_klein_4b_distilled_edit.json` — optional local ComfyUI engine. Zero pins preserve trusted T2I; one concrete Approved `PinnedIdentityAsset` is exact-key validated/read, uploaded through `/upload/image`, and consumed only by the trusted Flux.2 Klein edit graph. `scripts/e2e-flux-identity-reference.sh` + `.demo/FluxIdentityReferenceProof/` are the USER-RUN three-scene visual proof (not run by coding agents).
...
- `tests/AIStudio.Tests/Architecture/ArchitectureBoundaryTests.cs` — cheap assembly-level guardrails for ADR-018 (Domain must not reference Application/Infrastructure/Api; Application must not reference Infrastructure/Api; Infrastructure must not reference Api); reflection only, no architecture-testing package.
...
- `docs/product/parts/13_EXTRACTABLE_MODULE_BOUNDARIES.md` — post-freeze addendum (not source-derived, does not change the source checksum): modular-first / extractable-module rules and ADR-018/ADR-019.
