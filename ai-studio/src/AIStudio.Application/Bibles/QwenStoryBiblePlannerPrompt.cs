...
public static class QwenStoryBiblePlannerPrompt
{
    ...
    public static string Build(CreativeDirection direction, StoryPlan storyPlan)
    {
        ...
        prompt.AppendLine()
            ...
            .AppendLine("- Never invent a more specific environment classification than the story establishes. If the narrative only gives a generic indoor room or play space, use the closest supported neutral environment type rather than guessing a different specific place such as a bedroom, kitchen, or classroom. The environment type must match the world's own name, description, and recurring props.")
            .AppendLine("- Create a characterBible only when an entity is a persistent narrative character or agent whose identity or behavior continuity matters across beats. Ordinary objects, scenery, tools, toys, furniture, and passive props belong in worldBibles.recurringProps and must not become characterBibles merely because they appear in several beats. An object may be a characterBible only when the story clearly personifies it or gives it autonomous narrative agency; agency, not species, decides.")
