...
namespace AIStudio.Domain.Narration;

public sealed class NarrationTrack
{
    public const int MaxPathLength = 1024;
    public const int MaxProvenanceLength = 500;
    public const int ContentHashLength = 64;

    private NarrationTrack()
    {
    }
    ...
    public static NarrationTrack Create(
        Guid contentProjectId,
        Guid sourceJobId,
        string path,
        long byteSize,
        ...
        Guid? id = null)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(origin), "Narration origin is not supported.");
        }

        var trackId = id ?? Guid.NewGuid();
        if (trackId == Guid.Empty)
        {
            throw new ArgumentException("Narration track ID cannot be empty.", nameof(id));
        }
        ...
        if (origin == AssetOrigin.External)
        {
            if (normalizedSource is null)
            {
                throw new ArgumentException(
                    "Externally sourced narration requires a source.",
                    nameof(source));
            }
            ...
            {
                throw new ArgumentException(
                    "Externally sourced narration requires license information.",
                    nameof(license));
            }
        }
