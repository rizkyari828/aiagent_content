...
public sealed class SceneAsset
{
    ...
    /// <summary>
    /// Replaces the mutable visual fields of a generated asset in place. Used when a
    /// scene is regenerated by a newer visual-direction version; the asset keeps its
    /// identity and source job, so no schema change is required.
    /// </summary>
    public void Replace(
        AssetType type,
        string path,
        ...
        string? creator)
    {
        ...
        Type = type;
        Path = RequireText(path, MaxPathLength, nameof(path));
        ByteSize = byteSize;
        ContentHash = RequireHash(contentHash);
        Source = OptionalText(source, MaxProvenanceLength, nameof(source));
        Creator = OptionalText(creator, MaxProvenanceLength, nameof(creator));
    }
