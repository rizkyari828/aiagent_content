...
namespace AIStudio.Domain.Subtitles;
...
public sealed class SubtitleTrack
{
    ...
    public static SubtitleTrack Create(
        Guid contentProjectId,
        Guid sourceJobId,
        string path,
        long byteSize,
        ...
        Guid? id = null)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(origin), "Subtitle origin is not supported.");
        }

        var trackId = id ?? Guid.NewGuid();
        if (trackId == Guid.Empty)
        {
            throw new ArgumentException("Subtitle track ID cannot be empty.", nameof(id));
        }
        ...
        if (origin == AssetOrigin.External)
        {
            if (normalizedSource is null)
            {
                throw new ArgumentException(
                    "Externally sourced subtitles require a source.",
                    nameof(source));
            }
            ...
            {
                throw new ArgumentException(
                    "Externally sourced subtitles require license information.",
                    nameof(license));
            }
        }
