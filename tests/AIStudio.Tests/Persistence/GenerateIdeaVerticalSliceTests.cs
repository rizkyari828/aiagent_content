...
public sealed class GenerateIdeaVerticalSliceTests
{
    [Fact]
    public async Task WorkerPipeline_PersistsStructuredGenerateIdeaResult()
    {
        ...
        var project = ContentProject.Create(
            $"GenerateIdea integration {Guid.NewGuid():N}",
            "Disposable AI vertical-slice row.",
            now);
        var job = Job.Create(
            project.Id,
            JobType.GenerateIdea,
            $"generate-idea-{Guid.NewGuid():N}",
            GenerateIdeaTestData.ValidPayload(project.Id),
            now,
            maxRetries: 0);
        ...
        try
        {
            ...
            var handler = new GenerateIdeaJobHandler(
                ...
                new RecordingTextGenerator(GenerateIdeaTestData.ValidResult));
            ...
            var result = GenerateIdeaResult.Deserialize(persisted.Result);
